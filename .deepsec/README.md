...
Currently configured project: `discord-bot` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/discord-bot/SETUP.md` to fill in
   `data/discord-bot/INFO.md`.
...
`--project-id discord-bot` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/discord-bot/`.
